...
metadata <- data.frame(sample = names(mat),
...
dir.create("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/")
write.csv(ITS_p, "/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/ITS_SigScore.csv", quote=F, row.names=F)
...
pred_en$data$prob.1
write.csv(pred_en$data, "/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/ITS_prediction_prob.csv", quote=F, row.names=F)
...
res$label = factor(res$label, level=c("vehicle", "JQ1", "antiPD1", "combo"))
p = ggplot(res, aes(label, prob.1, fill = label)) +
        ...
        theme_bw()
# p
ggsave("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/prediction_boxplot.pdf", p, width=5, height=4)

my_comparisons1 <- list(c("vehicle", "JQ1")) 
my_comparisons2 <- list(c("vehicle", "antiPD1"))
my_comparisons3 <- list(c("vehicle", "combo"))
my_comparisons4 <- list(c("JQ1", "antiPD1"))
my_comparisons5 <- list(c("JQ1", "combo"))
my_comparisons6 <- list(c("antiPD1", "combo"))
...
p = ggplot(res, aes(label, prob.1, fill = label)) +
        ...
        stat_compare_means(method="t.test",hide.ns = F,
                            ...
                            bracket.size=0.5,
                            size=2.5)
ggsave("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/prediction_boxplot_t.test.pdf", p, width=5, height=4)


data_test$sample=rownames(data_test)
data_test=merge(metadata,data_test)
data_test$label = factor(data_test$label, c("vehicle", "JQ1", "antiPD1", "combo"))

immuneSig_boxplot = lapply(as.list(names(data_test)[-c(1,2,60,50)]), function(x){
    # x = "mhc_ips"
...
pdf("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/singleITS.pdf",
            width = 5,
            height = 4, 
            onefile = TRUE)
...
pdf("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/result_GSE114601/ICB_prediction/singleITS_t.test.pdf",
            width = 5,
            height = 4, 
            onefile = TRUE)
for(x in seq(length(immuneSig_boxplot))){
