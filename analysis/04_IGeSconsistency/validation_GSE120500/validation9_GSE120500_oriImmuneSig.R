...
count <- as.data.frame(read_excel("data/drug_pertubation_animal/GSE120500/GSE120500_Mouse_Transcriptome_control_and_parpi.xls", 
                     sheet = 1))
...
TIDEweb_res <- read.csv(paste0(result_dir, "TIDEweb/GSE120500_yestoPD1treatment.csv"), row.names=1)
...
names(metadata) = c("title","geo_accession","treatment","patient")
merged_res = inner_join(metadata, merged_res)
immuneSig_boxplot = lapply(as.list(names(merged_res)[-c(1:4)]), function(x){
    # x = "CD8 T cells_mcp_counter"
    tmp = merged_res[c("patient","treatment", x)]
    names(tmp) = c("patient", "label", "score")
    tmp$label = factor(tmp$label, c("Vehicle", "Olaparib"))
    tmp1 = tmp[grep("Vehicle", tmp$label),]
    tmp2 = tmp[grep("Olaparib", tmp$label),]
    pval = wilcox.test(tmp1$score, tmp2$score)$p.value
    p <- ggplot(tmp, aes(x=label, y=score, fill=label)) + 
        geom_boxplot() + 
        geom_jitter(shape=16, position=position_jitter(0.2))+
        ggtitle(paste0(x, "\nOlaparibvsVehicle p = ", round(pval, 4))) + 
        theme_bw()
    # p
    return(p)
})
...
merged_res_selected <- merged_res[c("patient", "treatment", intersect(names(merged_res), immunesig$immune_sig))]


immuneSig_selected_boxplot = lapply(as.list(names(merged_res_selected)[-c(1:4)]), function(x){
    # x = "CD8 T cells_mcp_counter"
    tmp = merged_res[c("patient","treatment", x)]
    names(tmp) = c("patient", "label", "score")
    tmp$label = factor(tmp$label, c("Vehicle", "Olaparib"))
