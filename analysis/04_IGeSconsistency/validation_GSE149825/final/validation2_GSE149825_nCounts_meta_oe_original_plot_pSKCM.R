...
gsea_resultpath = paste0("result_", GEO_ID, "/drug_immunesig_", purity_method,"_GSEA_result_r",s1,"_meta_oe_original/")
...
savepath = paste0("result_", GEO_ID, "/drug_immunesig_", purity_method,"_GSEA_result_r",s1,"_meta_oe_original_en_padj",Pval,"_final/", drugindex)
dir.create(savepath)
...
Pval = 0.1 
dir.create(paste0("result_", GEO_ID, "/drug_immunesig_", purity_method,"_GSEA_result_r",s1,"_meta_oe_original_en_padj",Pval,"/"))
savepath = paste0("result_", GEO_ID, "/drug_immunesig_", purity_method,"_GSEA_result_r",s1,"_meta_oe_original_en_padj",Pval,"/", drugindex)
dir.create(savepath)
...
immunesig <- read.csv("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r1_drug_immuneSig_correlation_analysis/05_immuneSig_ICBresponse/results/meta_results/SKCM_res_metaRes.csv")
immunesig <- immunesig[immunesig$Meta_Pval < 0.05, ]
names(immunesig) <- c("immune_sig", names(immunesig)[-1])
immunesig$flag <- "sensitive"
immunesig[immunesig$OR < 1,]$flag <- "resistant"
...
# immunesig_ITS_index <- read.table(paste0("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r1_drug_immuneSig_correlation_analysis/03_drug_immuneSig_enrichment/data/ITS_mclust_Otsuka_Ochiai_CPE_spearman_r_0.4_metaP_0.05_genevote_0.5/",cancer,"_immunesig_ITS_index.txt"), sep = "\t", header = T)
# immunesig <- merge(immunesig_ITS_index[-3], immunesig, by='immune_sig', all.y = T)

savepath = paste0("06_results_summaryandplotting/results_meta_oe_original/", purity_method,"_",model,"_meta_weight_ACATP0.05/") #
dir.create(savepath)
  dir.create(paste0(savepath, "/"))
  dir.create(paste0(savepath, "/",dataset,"/"))
  dir.create(paste0(savepath, "/",dataset,"/", datatype,"/"))
...
savepath2 = "/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r3_individual_datasets_validation/data/drugITS_metaSKCM_oe_original/"
dir.create(savepath2)
drugITGSscore_preparation(df_immunesig = df_immunesig,
                          cancer = cancer,
                          tumor_subtype = tumor_subtype,
...
# ITSselected <- read.csv("06_results_summaryandplotting/data/immune_sig_selected_new/selected_ITS_0.4.txt", sep = '\t')
meta_res <- read.csv("05_immuneSig_ICBresponse/results/meta_results/SKCM_res_metaRes.csv")
meta_res_selected <- meta_res[meta_res$Meta_Pval < 0.05, ]
sig_s = meta_res_selected[meta_res_selected$OR > 1,]
sig_r = meta_res_selected[meta_res_selected$OR < 1,]
sig_s = data.frame( immune_sig = sig_s$X, flag = "sensitive", setindex = '01_sensitive')
...
savepath = paste0("result_", GEO_ID, "/drug_immunesig_", purity_method,"_GSEA_result_r",s1,"_meta_oe_original/", drugindex,"_SKCM")
dir.create(paste0("result_", GEO_ID))
dir.create(paste0("result_", GEO_ID, "/drug_immunesig_", purity_method,"_GSEA_result_r",s1,"_meta_oe_original/"))
dir.create(savepath)
