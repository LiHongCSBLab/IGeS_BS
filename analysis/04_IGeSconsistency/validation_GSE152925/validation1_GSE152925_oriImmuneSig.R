...
# load data
# count <- read.csv(gzfile("data/immune_combo_dataset/GSE152925/GSE152925_B16_C57BL6_invivo_RawCount.txt.gz"), 
#                         sep = '\t', row.names = 1)
...
tpm_all <- read.csv(gzfile("data/immune_combo_dataset/GSE152925/GSE152925_B16_C57BL6_invivo_TPM.txt.gz"), 
                        sep = '\t', row.names = 1)
# metadata = metadata_all[is.element(metadata_all$`treatment:ch1`, 
#                         c("Swainsonine +IgG", "IgG +PBS","anti-PD-L1","anti-PD-L1+Swainsonine")), ]
tpm <- tpm_all[is.element(names(tpm_all),  metadata_all$description)]
...
metadata = metadata_all[is.element(metadata_all$description, colnames(tpm_mat)),][c("description", "treatment:ch1")]
names(metadata) = c("patient", "treatment")
...
TIDEweb_res <- read.csv(paste0(result_dir, "TIDEweb/GSE152925_yestoPD1treatment.csv"), row.names=1)
...
merged_res = inner_join(metadata, merged_res)
immuneSig_boxplot = lapply(as.list(names(merged_res)[-c(1,2)]), function(x){
    # x = "CD8 T cells_mcp_counter"
    tmp = merged_res[c("patient","treatment", x)]
    names(tmp) = c("patient", "label", "score")
    tmp$label = factor(tmp$label, c("IgG +PBS", "Swainsonine +IgG", "anti-PD-L1+PBS", "anti-PD-L1+Swainsonine"))
    tmp1 = tmp[tmp$label == "IgG +PBS",]
    tmp2 = tmp[tmp$label == "Swainsonine +IgG",]   
    pval = wilcox.test(tmp1$score, tmp2$score)$p.value
    p <- ggplot(tmp, aes(x=label, y=score, fill=label)) + 
        geom_boxplot() + 
        geom_jitter(shape=16, position=position_jitter(0.2))+
        ggtitle(paste0(x, "\nSwainsonine+IgG vs IgG+PBS p = ", round(pval, 4))) + 
        theme_bw()
    # p
    return(p)
})
...
merged_res_selected <- merged_res[c("patient", "treatment", immunesig$immune_sig)]


immuneSig_selected_boxplot = lapply(as.list(names(merged_res_selected)[-c(1,2)]), function(x){
    # x = "CD8 T cells_mcp_counter"
    tmp = merged_res_selected[c("patient","treatment", x)]
    names(tmp) = c("patient", "label", "score")
    tmp$label = factor(tmp$label, c("IgG +PBS", "Swainsonine +IgG", "anti-PD-L1+PBS", "anti-PD-L1+Swainsonine"))
    tmp1 = tmp[tmp$label == "IgG +PBS",]
    tmp2 = tmp[tmp$label == "Swainsonine +IgG",]   
