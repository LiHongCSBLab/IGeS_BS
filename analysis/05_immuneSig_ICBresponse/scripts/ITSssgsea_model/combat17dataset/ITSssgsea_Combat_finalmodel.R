...
pred_rf_res <- pred_rf$data
tmp = pred_rf_res[grep('96619', rownames(pred_rf_res)),]
...
pred_xgboost_res <- pred_xgboost$data
tmp = pred_xgboost_res[grep('Liu', rownames(pred_xgboost_res)),]
pred2 <- ROCR::prediction(tmp$prob.1 , tmp$truth)
aucPerf <- ROCR::performance( pred2, 'auc' )
AUCValue<-aucPerf@y.values[[1]]
AUCValue
...
pred_naiveBayes_res <- pred_naiveBayes$data
tmp = pred_naiveBayes_res[grep('Lau', rownames(pred_naiveBayes_res)),]
pred2 <- ROCR::prediction(tmp$prob.1 , tmp$truth)
aucPerf <- ROCR::performance( pred2, 'auc' )
AUCValue<-aucPerf@y.values[[1]]
AUCValue
