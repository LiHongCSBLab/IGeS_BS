...
resultpath = paste0(result_dir, "modelCV_alldatasets_ssgseanorm/")
...
modeling_task <- lapply(as.list(names(folds)), function(i){
  ...
  pred <- predict(ridge.fit, s = bestlam, newx= as.matrix(data_test[-120]))
  pred <- cbind(pred, data_test[120])
...
resultpath = paste0(result_dir, "finalmodel_alldatasets_ssgseanorm/")
dir.create(resultpath)
...
tmp = pred[grep('96619', rownames(pred)),]
...
# ridge.fit <- glmnet(data_train_filter[-120], as.numeric(data_train_filter$tag), alpha = 0, lambda=grid)
# cv.ridge <- cv.glmnet(as.matrix(data_train_filter[-120]), as.numeric(data_train_filter$tag), alpha =0, lambda=grid)
# ridge_model_para = cv.ridge$glmnet.fit
# m_ridge_imp <- data.frame(ridge_model_para$beta[,which(ridge_model_para$lambda == cv.ridge$lambda.min)])
# names(m_ridge_imp) <- 'feature_weight'
# m_ridge_imp$feature_name <- rownames(m_ridge_imp)
...
# tmp = pred[grep('115821', rownames(pred)),]
# pred2 <- ROCR::prediction(tmp$prob.1 , tmp$tag)
# aucPerf <- ROCR::performance( pred2, 'auc' )
# AUCValue<-aucPerf@y.values[[1]]
# AUCValue