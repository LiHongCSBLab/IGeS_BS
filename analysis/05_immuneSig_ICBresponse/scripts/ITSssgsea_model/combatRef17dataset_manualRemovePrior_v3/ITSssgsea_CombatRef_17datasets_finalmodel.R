...
# command=matrix(c( 
#   "featureFilterStrategy",  "s", 1, "character", "featureFilterStrategy in metaAnalysis_removeprior_v3, combinedP, voting, mlrFilter",
...
m_en_imp <- inner_join(m_en_imp, meta_selected)
plot(m_en_imp$en_weight, m_en_imp$OR)
abline(v=0,h=1)

m_en_imp <- m_en_imp[m_en_imp$en_weight !=0,]
m_en_imp[order(m_en_imp$en_weight, decreasing = T),][m_en_imp$OR >0,]
