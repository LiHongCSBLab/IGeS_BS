...
result_dir <- "05_immuneSig_ICBresponse/results_mouse_v202303_noWT/"
...
data_mouse <- read.csv("05_immuneSig_ICBresponse/data/mouse_zhao/v202303_noWT/TPMmat_TPM.txt", sep = '\t', header = T, row.names = 1) 
...
mouseMetaInfo <- read.csv("05_immuneSig_ICBresponse/data/mouse_zhao/v202303/mMetadataMerged.csv")
...
sig_select = inner_join(sig_select, m_en_imp)
rownames(sig_select) = sig_select$immune_sig
data_test_selected_p <- data_test[c(intersect(sig_select[sig_select$weight!=0,]$immune_sig, names(data_test)))]
pdf(paste0(savepath, "/metaAnalysis_filter/mouseITS_v202303_noWT_", enrich_method, "_IGeS.pdf"), 
    width = 15, height = 15)
pheatmap(data_test_selected_p, scale = 'column', 
         cluster_col = F,
         annotation_row = mouseMetaInfo["Genotype"],
         annotation_col = sig_select[sig_select$weight!=0,][c("flag","Type")])
dev.off()
...
ggsave(paste0(savepath, "/metaAnalysis_filter/mouseITS_v202303_noWT_", enrich_method, "_pred.pdf"), p, width = 10, height = 5)
write.csv(pred_mouse, paste0(savepath, "/metaAnalysis_filter/mouseITS_v202303_noWT_", enrich_method, "_enPred.csv"),
             row.names =F, quote = F)
...
data_test_selected_p <- data_test[c(intersect(sig_select[sig_select$weight!=0,]$immune_sig, names(data_test)))]
pdf(paste0(savepath, "/metaAnalysis_filter_removeprior_v2/mouseITS_v202303_noWT_", enrich_method, "_IGeS.pdf"), 
    width = 15, height = 15)
pheatmap(data_test_selected_p, scale = 'column', 
         cluster_col = F,
         annotation_row = mouseMetaInfo["Genotype"],
...
p = ggplot(pred_mouse, aes(x=Genotype, y=prob.1)) +
      ...
      theme_bw()+   
      theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(paste0(savepath, "/metaAnalysis_filter_removeprior_v2/mouseITS_v202303_noWT_", enrich_method, "_pred.pdf"), p, width = 10, height = 5)
write.csv(pred_mouse, paste0(savepath, "/metaAnalysis_filter_removeprior_v2/mouseITS_v202303_noWT_", enrich_method, "_enPred.csv"),
             row.names =F, quote = F)
...
pdf(paste0(savepath, "/metaAnalysis_filter_removeprior_v3/mouseITS_v202303_noWT_", enrich_method, "_IGeS.pdf"), 
    width = 15, height = 15)
pheatmap(data_test_selected_p, scale = 'column', 
         cluster_col = F,
         annotation_row = mouseMetaInfo["Genotype"],
...
p = ggplot(pred_mouse, aes(x=Genotype, y=prob.1)) +
      ...
      theme_bw()+   
      theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(paste0(savepath, "/metaAnalysis_filter_removeprior_v3/mouseITS_v202303_noWT_", enrich_method, "_pred.pdf"), p, width = 10, height = 5)
write.csv(pred_mouse, paste0(savepath, "/metaAnalysis_filter_removeprior_v3/mouseITS_v202303_noWT_", enrich_method, "_enPred.csv"),
             row.names =F, quote = F)
...
rownames(sig_select) = sig_select$immune_sig
data_test_selected_p <- data_test[c(intersect(sig_select[sig_select$weight!=0,]$immune_sig, names(data_test)))]
pdf(paste0(savepath, "/metaAnalysis_filter_removeprior_v4/mouseITS_v202303_noWT_", enrich_method, "_IGeS.pdf"), 
    width = 15, height = 15)
pheatmap(data_test_selected_p, scale = 'column', 
         cluster_col = F,
         annotation_row = mouseMetaInfo["Genotype"],
...
p = ggplot(pred_mouse, aes(x=Genotype, y=prob.1)) +
      ...
      theme_bw()+   
      theme(axis.text.x = element_text(angle = 90, hjust = 1))
ggsave(paste0(savepath, "/metaAnalysis_filter_removeprior_v4/mouseITS_v202303_noWT_", enrich_method, "_pred.pdf"), p, width = 10, height = 5)
write.csv(pred_mouse, paste0(savepath, "/metaAnalysis_filter_removeprior_v4/mouseITS_v202303_noWT_", enrich_method, "_enPred.csv"),
             row.names =F, quote = F)
...
# sig_select <- immunesig[immunesig$Meta_Pval < 0.05, ]

# features_selected <- sig_select$X
# data_test = ITSp_res[ITSp_res$MLbatch == 'test', ]
# # rownames(data_test) = data_test$SampleID
# data_test$pdc_xcell=0
# data_test$monocyte_immucellai=0
# data_test$macrophage.m1_cibersort=0

# data_test <- data_test[c(intersect(m_en$features, names(data_test)))]
...
# pred_en = predict(m_en, task = mouse_testtask)
# pred_mouse = pred_en$data
# pred_mouse$SampleID = rownames(pred_mouse)
# pred_mouse <- merge(mouseMetaInfo, pred_mouse, by = 'SampleID' )
# pred_mouse = pred_mouse[order(pred_mouse$prob.1, decreasing = T), ]
# pred_mouse[c("SampleID","Genotype","prob.1" )]

# gt_score = aggregate(pred_mouse['prob.1'], by = list(pred_mouse$Genotype), median)
# gt_score[order(gt_score$prob.1, decreasing = T), ]
