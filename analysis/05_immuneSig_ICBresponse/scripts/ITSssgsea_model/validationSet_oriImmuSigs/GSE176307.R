...
result_path <- "05_immuneSig_ICBresponse/validation_results/"
...
result_dir = "05_immuneSig_ICBresponse/validation_results/GSE176307/"
...
res = read.csv(paste0(result_dir, "TIDE_TIP_result/TIDE_TIP_result_BLCA_Primary.csv" ), row.names=1)
...
TIDEweb_res <- read.csv(paste0(result_dir, "TIDEweb/GSE176307.csv"), row.names=1)
...
immuneSig_sig160_res$patient <- gsub("\\.","-",immuneSig_sig160_res$patient)
...
icb_compr(result = merged_res,
          response = response,
          resultdir = "05_immuneSig_ICBresponse/validation_results/immunesig_icbResponse_test_results/",
          filename = "GSE176307_test_result.csv")



immuneSig_res <- read.csv("05_immuneSig_ICBresponse/validation_results/immunesig_icbResponse_test_results/GSE176307_test_result.csv", row.names = 1)
...
write.csv(res, 
          "05_immuneSig_ICBresponse/validation_results/immunesig_icbResponse_all_results/GSE176307_test_result.csv",
          row.names = F, quote = F)



merged_res <- read.csv(paste0(result_dir, "/", cancer, "_", sample_type, "_immuneSig_merged.csv"))
merged_res$patient = gsub("-", "_", merged_res$patient)
names(response) = c('patient', 'label')
response <- response[!is.na(response$label), ]
response$tag = 0
response[response$label == 'R',]$tag = 1
res_df <- merge(response, merged_res,by='patient')
names(res_df) <- tolower(names(res_df))
signames <- names(res_df)
signames <- signames[-which(is.element(signames, c("patient","label", "dataset", "cancer", "sample_type" ,"tag")))]
othermodel_auc <- list()
signames <- c('tide_tide', 'cd8_tide', 'ifng_tide','merck18_tideweb', 'ips_ips', 'tip_signature_tip', 'oe_resu')
for(i in 1: length(signames)){
  df = res_df[c(signames[i], "tag")]
  names(df) = c("score", "tag")
  # df$pred = 0
  # df[df$score <0,]$pred = 1
#   pred2 <- ROCR::prediction(df$score, df$tag)
#   aucPerf <- ROCR::performance( pred2, 'auc' )
#   AUCValue<- data.frame(method = aucPerf@y.values[[1]])
#   othermodel_auc[i] <- AUCValue

  
    modelroc <- pROC::roc(df$tag, df$score)
    othermodel_auc[i] <-  modelroc$auc[1]

}

names(othermodel_auc) <- signames
othermodel_auc <- as.data.frame(do.call(rbind, othermodel_auc))
names(othermodel_auc) = 'auc'
othermodel_auc$immune_sig <- rownames(othermodel_auc)
othermodel_auc <- othermodel_auc[order(othermodel_auc$auc, decreasing = T), ]
othermodel_auc[othermodel_auc$auc > 0.6,]
othermodel_auc[othermodel_auc$immune_sig == 'tide_tide',]

write.csv(othermodel_auc, "05_immuneSig_ICBresponse/validation_results/GSE176307_othermodel_AUC.csv", quote = F, row.names = F)
