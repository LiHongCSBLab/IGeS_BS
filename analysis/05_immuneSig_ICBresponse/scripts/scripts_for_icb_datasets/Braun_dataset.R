...
TIDEweb_res <- read.csv(paste0(result_dir, "TIDEweb/Braun_dataset.csv"), row.names=1)
...
merged_res <- merged_res[is.element(merged_res$patient, response$patient), ]
...
write.csv(res, 
          "05_immuneSig_ICBresponse/results/immunesig_icbResponse_all_results/Braun_dataset_PD1_test_result.csv",
          row.names = F, quote = F)
