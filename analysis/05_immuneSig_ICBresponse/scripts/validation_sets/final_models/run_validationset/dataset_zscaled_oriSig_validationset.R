...
# ------------------------------------------------------------------------------
resultpath = "05_immuneSig_ICBresponse/validation_results/oriImmuSig_mlr_model/alldatasets_zscaled/"
...
data_test_scaled <- apply(data_test[-c(1:2)], 2, scale)
data_test <- cbind(data_test[1:2], data_test_scaled)
...
# ------------------------------------------------------------------------------
load(paste0(resultpath, "/en/en_model_pred.Rdata"))
...
save(elasticnet_opt_ps_res, mod_elasticnet_opt, m_en, pred_en, 
     file = paste0(savepath, "/en_model_pred.Rdata"))
