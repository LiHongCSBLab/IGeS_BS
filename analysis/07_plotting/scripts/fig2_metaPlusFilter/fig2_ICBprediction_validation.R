...
resultpath = paste0(result_dir, "finalmodel_17datasets_ssgseanorm/metaAnalysis_filter/")
...
write.csv(vlSetName_auc_all, "07_plotting_v2/02_ITSprediction_metaPlusFilter/vlSetName_auc_all.csv", quote = F, row.names = F)
write.csv(vlSetName_auc, "07_plotting_v2/02_ITSprediction_metaPlusFilter/vlSetName_auc.csv", quote = F, row.names = F)
...
for(i in seq(length(models))){
    ...
    modelroc[[i]] <- pROC::roc(res_pred$tag, res_pred$prob.1)
    
}
names(modelroc)= c("ridge", "elastic net", "random forest", "svm", "xgboost")
p=pROC::ggroc(modelroc) + theme_bw()
ggsave("07_plotting_v2/02_ITSprediction_metaPlusFilter/vlSetName_roc_all.pdf",p, width=8, height=7)
...
ggsave("07_plotting_v2/02_ITSprediction_metaPlusFilter/fig_s2_vlSetName.pdf", p_s2b, width = 8, height = 6)
...
ROC_VAL=c(modelroc[2], ROC_VAL)
p=pROC::ggroc(ROC_VAL) + theme_bw()
ggsave("07_plotting_v2/02_ITSprediction_metaPlusFilter/vlSetName_roc_otherModel.pdf",p, width=8, height=7)
...
write.csv(singledataset_AUC_selected, "07_plotting_v2/02_ITSprediction_metaPlusFilter/vlSetName_auc_otherModel.csv", quote = F)
...
ggsave("07_plotting_v2/02_ITSprediction_metaPlusFilter/fig_2cd_vlSetName.pdf", p_fig2cd, width = 15, height = 18)
