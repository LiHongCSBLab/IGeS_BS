...
names(survival) = c( "patientID","pfs","pfs_Status","os","os_Status", "irPFS","irPFS_Status")

# cox regression

df_pred = res_pred[grep(dset, res_pred$SampleID),]
df_pred$patientID = gsub("Braun_dataset", "", df_pred$SampleID)
...
# irPFS ---------------------------------------------------------------------------
# real response and survival
fit_irPFS <- survfit(Surv(irPFS, irPFS_Status) ~ label, data = y)
# summary(fit_irPFS)
p_irPFS = ggsurvplot(fit_irPFS, # 创建的拟合对象
           data = y,  # 指定变量数据来源
           conf.int = TRUE, # 显示置信区间
           pval = TRUE, # 添加P值
           # surv.median.line = "hv",  # 添加中位生存时间线
           risk.table = TRUE,
            xlab = "Months")  # 设置x轴刻度间距
# print(p_irPFS)
pdf(paste0(plot_savepath, "/",dset,"/irPFS_dataset_TrueResponse.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
y$response_pred = 'NR'
y[y$prob.1 > median(y$prob.1), ]$response_pred = 'R'
fit_irPFS <- survfit(Surv(irPFS, irPFS_Status) ~ response_pred, data = y)
# summary(fit_irPFS)
p_irPFS = ggsurvplot(fit_irPFS, # 创建的拟合对象
           data = y,  # 指定变量数据来源
           conf.int = TRUE, # 显示置信区间
...
# print(p_irPFS)
# dev.off()
pdf(paste0(plot_savepath, "/",dset,"/irPFS_dataset_pred_meidan.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
p_irPFS = ggsurvplot(fit_irPFS, # 创建的拟合对象
           ...
           risk.table = TRUE,
            xlab = "Months")  # 设置x轴刻度间距
# print(p_irPFS)
pdf(paste0(plot_savepath, "/",dset,"/irPFS_dataset_pred_mean.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
p_irPFS = ggsurvplot(fit_irPFS, # 创建的拟合对象
           ...
           risk.table = TRUE,
            xlab = "Months")  # 设置x轴刻度间距
# print(p_irPFS)
pdf(paste0(plot_savepath, "/",dset,"/irPFS_dataset_pred_SurvBestCutoff.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
p_irPFS = ggsurvplot(fit_irPFS, # 创建的拟合对象
           ...
           risk.table = TRUE,
            xlab = "Months")  # 设置x轴刻度间距
# print(p_irPFS)
pdf(paste0(plot_savepath, "/",dset,"/irPFS_dataset_pred_ROCBestCutoff_youden.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
p_irPFS = ggsurvplot(fit_irPFS, # 创建的拟合对象
           ...
           risk.table = TRUE,
            xlab = "Months")  # 设置x轴刻度间距
# print(p_irPFS)
pdf(paste0(plot_savepath, "/",dset,"/irPFS_dataset_pred_ROCBestCutoff_closesttopleft.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
# print(p_irPFS)
# dev.off()
pdf(paste0(plot_savepath, "/",dset,"/irPFS_global_pred_ROCBestCutoff_youden.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
...
# print(p_irPFS)
# dev.off()
pdf(paste0(plot_savepath, "/",dset,"/irPFS_global_pred_ROCBestCutoff_closesttopleft.pdf"))
print(p_irPFS, newpage = FALSE)
dev.off()
