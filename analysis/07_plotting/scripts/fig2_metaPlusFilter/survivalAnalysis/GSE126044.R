...
df_pred$patientID = gsub("GSE126044", "", df_pred$SampleID)

names(survival) = c("patientID", "pfs", "os")
survival$pfs = as.numeric(survival$pfs)
...
fit_os <- survfit(Surv(os) ~ label, data = y)
# summary(fit_os)
...
fit_os <- survfit(Surv(os) ~ response_pred, data = y)
# summary(fit_os)
p_os = ggsurvplot(fit_os, # 创建的拟合对象
           data = y,  # 指定变量数据来源
           conf.int = TRUE, # 显示置信区间
...
# pfs ---------------------------------------------------------------------------
# real response and survival
fit_pfs <- survfit(Surv(pfs) ~ label, data = y)
# summary(fit_pfs)
...
y$response_pred = 'NR'
y[y$prob.1 > median(y$prob.1), ]$response_pred = 'R'
fit_pfs <- survfit(Surv(pfs) ~ response_pred, data = y)
# summary(fit_pfs)
p_pfs = ggsurvplot(fit_pfs, # 创建的拟合对象
           data = y,  # 指定变量数据来源
           conf.int = TRUE, # 显示置信区间
