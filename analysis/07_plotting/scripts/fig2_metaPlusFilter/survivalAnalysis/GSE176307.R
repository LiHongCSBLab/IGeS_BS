...
load(paste0(processedDataPath, "/GSE176307/processedData/", dataset))
survival = survival[c("patientID","OS","alive","PFS")]

survival$status = survival$alive
...
survival$pfs_Status = as.numeric(survival$pfs_Status)

# cox regression
df_pred = res_pred[grep(dset, res_pred$SampleID),]
df_pred$patientID = gsub("GSE176307", "", df_pred$SampleID)
