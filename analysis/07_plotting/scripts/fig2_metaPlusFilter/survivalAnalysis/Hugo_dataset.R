...
load(paste0(processedDataPath, "/Hugo_dataset/processedData/", dataset))

# cox regression
df_pred = res_pred[grep(dset, res_pred$SampleID),]
df_pred$patientID = gsub("01_Hugo_dataset_outcome", "", df_pred$SampleID)
length(intersect(df_pred$patientID, survival$patientID))
y = merge(df_pred, survival, by = 'patientID')
# survival: overall survival (days)
y$os = as.numeric(y$`Overall Survival`)
y$Status = 0
y[y$`Vital Status` == 'Dead', ]$Status = 1
