...
load(paste0(processedDataPath, "Nathanson_dataset/processedData/", dataset))
cancer = "SKCM"
...
survival[survival$Alive == 1, ]$os_Status = 0
names(survival) = c("patientID", "os", "Alive", "os_Status")
survival$os = as.numeric(survival$os)
# cox regression
df_pred = res_pred[grep(dset, res_pred$SampleID),]
df_pred$patientID = gsub("Nathanson_dataset_outcome", "", df_pred$SampleID)
