...
# dataset= "GSE91061.Rdata"
load(paste0(processedDataPath, "/Riaz_dataset/processedData/", dataset))


# cox regression
df_pred = res_pred[grep(dset, res_pred$SampleID),]
df_pred$patientID = gsub("02_Riaz_dataset_outcome", "", df_pred$SampleID)
length(intersect(df_pred$patientID, survival$patientID))
y = merge(df_pred, survival, by = 'patientID')
# survival: os (Weeks)
y$`os` = as.numeric(y$`os`)
y$Status = 0
y[y$Dead == TRUE, ]$Status = 1


# real response and survival
fit_os <- survfit(Surv(os, Status) ~ label, data = y)
summary(fit_os)
...
fit_os <- survfit(Surv(os, Status) ~ response_pred, data = y)
summary(fit_os)
p_os = ggsurvplot(fit_os, # 创建的拟合对象
           data = y,  # 指定变量数据来源
           conf.int = TRUE, # 显示置信区间
...
p_os = ggsurvplot(fit_os, # 创建的拟合对象
           ...
           risk.table = TRUE,
            xlab = "Weeks")  # 设置x轴刻度间距
# print(p_os)
# ggsave(paste0(plot_savepath, "/",dset,"/os_dataset_pred_meidan.pdf"), plot=print(p_os))
...
# fit_os <- survfit(Surv(`Overall Survival`, `Vital Status` == 'Dead') ~ response_pred, data = y)
fit_os <- survfit(Surv(os, Status) ~ response_pred, data = y)
summary(fit_os)
p_os = ggsurvplot(fit_os, # 创建的拟合对象
           data = y,  # 指定变量数据来源
...
# cox_m_os <- tryCatch(coxph(Surv(y$`os`, y$Status) ~ response_pred, data = y), error=function(x){return('error')})
# # hypothesis test
