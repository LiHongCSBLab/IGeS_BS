...
survival = survival[c("patientID", "OS (days) from PD1 Inhibitor", # "OS (months)","OS (mo)", 
                                   "Dead (1) or alive (0)",
                                   "PFS days from PD1 Inhibitor (calc)", # "PFS months (calc)", 
                                   "Progression on PD1 Inhibitor yes (1) or No (0)")]
names(survival) = c("patientID", "os", "os_Status", 
                    "pfs", "pfs_Status")

# cox regression
df_pred = res_pred[grep(dset, res_pred$SampleID),]
df_pred$patientID = gsub("Zhao_dataset", "", df_pred$SampleID)
