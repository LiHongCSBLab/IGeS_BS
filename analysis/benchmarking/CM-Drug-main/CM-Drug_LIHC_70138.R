...
for (indext_iii in 1) {
  ...
  # tempPath = sprintf('GSE92742_Broad_LINCS_gene_info.txt', dir_kkk)
  ...
  metadata$Num = metadata$inst_id # paste0(metadata$cell_id, "_",seq(1, nrow(metadata))) 
...
# )
inPath = c('Data'='GSE70138_Broad_LINCS_Level3_INF_mlr12k_n345976x12328.gctx')
...
setwd("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r1_drug_immuneSig_correlation_analysis/07_plotting_v2/CM_drug_res/")
# load("./result/cp/template.RData")
load("./template.RData")

fgsea.res.trt_cp <- list()
for(i in 1:1){fgsea.res.trt_cp[[i]] <- readRDS(paste(".//result/cp/c",i,"_fgsea.res.trt_cp_super.RDS",sep = ""))}
...
library(openxlsx)
openxlsx::write.xlsx(x = drug.choose.detail , file = "./result/cp/70138_results_with_CM_Drug.xlsx",
...
#save in excel file-type
library(openxlsx)
openxlsx::write.xlsx(x = drug.choose.detail , file = "./result/cp/70138_all_results_with_CM_Drug.xlsx",
                     sheetName = "screenResult", rownames = FALSE)

dim(drug.choose.detail)