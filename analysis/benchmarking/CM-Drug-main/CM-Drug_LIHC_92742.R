#### If you have any questions or are interested in collaborating, 
# please feel free to contact us at hsiayun@foxmail.com. Let's work together to advance the field of immunotherapy.
...
# Global setting
# Output
outDir = './Data/Compound_Data_92742/'
...
#   tempPath = sprintf('pertInfo.%s', dir_kkk)
  # tempPath = sprintf('instinfo_beta.txt', dir_kkk)
  # tempPath = sprintf('GSE70138_Broad_LINCS_inst_info.txt', dir_kkk)
  tempPath = sprintf('GSE92742_Broad_LINCS_inst_info.txt', dir_kkk)

  metadata = fread(input = tempPath, sep = '\t', header = TRUE, data.table = FALSE)

  metadata = metadata[metadata$cell_id == 'HEPG2',]
    #   metadata$rna_plate = substr(metadata$det_plate,1,19)
    #   metadata$rna_well = metadata$det_well
  metadata$Num = metadata$distil_id # paste0(metadata$cell_id, "_",seq(1, nrow(metadata))) 
...
# )
inPath = c('Data'='GSE92742_Broad_LINCS_Level3_INF_mlr12k_n1319138x12328.gctx')
...
metadataDir = './Data/Compound_Data_92742/'
...
# workaround
# gctxMatrix <- parse.gctx(inPath, cid = sampleDF$Num)
# gctxMatrix=gctxMatrix@mat[referenceRow, ]
...
CM_Drug.data <- readRDS("./Data/Compound_Data_92742/CM_Drug.data")
CM_Drug.data.gtc<- new("GCT", mat=CM_Drug.data)
write_gctx(CM_Drug.data.gtc, 
          compression_level = 9,
          "./Data/Compound_Data_92742/CM_Drug.data.gctx",
          appenddim = FALSE)

rm(list = ls())
# setwd("~/tools/repository/CM-Drug")
CM_Drug.condition <- readRDS("./Data/Compound_Data_92742/CM_Drug.condition")
...
for(i in 1:1)trt_cp_number.group[[i]] <- trt_cp_number#[((i-1)*10000+1):(i*10000)]}

# trt_cp_number.group[[5]] <- trt_cp_number[40001:length(trt_cp_number)]

template <- parse_gctx("Data/Compound_Data_92742/CM_Drug.data.gctx",
...
for(j in c(1:1)){
  ...
  data_trt_cp[[j]] <- parse_gctx("./Data/Compound_Data_92742/CM_Drug.data.gctx", 
  ...
  setwd("./result_92742/cp/");saveRDS(fgsea.res.trt_cp[[j]],paste("c",j,"_fgsea.res.trt_cp_super.RDS",sep = ""));rm(list=ls());
}
...
# setwd("~/tools/repository/CM-Drug/result_92742/cp/")
# load("~/tools/repository/CM-Drug/template.RData")
setwd("/picb/bigdata/project/FengFYM/p2_1_immunotherapy_targetedtherapy/r1_drug_immuneSig_correlation_analysis/07_plotting_v2/CM_drug_res/")
# load("./result_92742/cp/template.RData")
 load("./template.RData")

fgsea.res.trt_cp <- list()
for(i in 1:1){fgsea.res.trt_cp[[i]] <- readRDS(paste(".//result_92742/cp/c",i,"_fgsea.res.trt_cp_super.RDS",sep = ""))}
...
fgsea.clue.order <- readRDS("./fgsea.clue.order_with_id.RDS")
...
#screening strategy2
#Considering that certain drugs significantly enhance specific pathways 
# but show slightly insufficient significance in one or two other pathways, 
# to prevent the oversight of such drugs, we have implemented an alternative 
# threshold screening allowing for some flexibility. Specifically, we allow an 
# adjusted p-value between 0.05 and 0.2 for enrichment results in two pathways, 
# while the adjusted p-value for other enriched pathways must be less than 0.05.
...
# for(i in 1:8){if(x[i]<1){count <- count+1}};return(count);count <- 0})
fgsea.clue.screen6.count <- purrr::map(fgsea.clue.screen6.row,function(x){count <- 0;
...
sig_info.choose.HEPG2<- sig_info.choose.fgsea %>% dplyr::filter(cell_id == "HEPG2")
...
library(openxlsx)
openxlsx::write.xlsx(x = drug.choose.detail , file = "./result_92742/cp/92742_results_with_CM_Drug.xlsx",
                     sheetName = "screenResult", rownames = FALSE)

################################################################################
# run for LIHC cell line
dim(sig_info.choose.fgsea)
head(drug.choose.detail,20)


# no filter
fgsea.clue.screen1 <- fgsea.clue.order
fgsea.clue.screen2 <- purrr::map(fgsea.clue.screen1,~  .x$id)
fgsea.clue.screen3 <- Reduce(intersect,fgsea.clue.screen2)
fgsea.clue.screen4 <- purrr::map(fgsea.clue.order,~ .x[as.integer(fgsea.clue.screen3),])
fgsea.clue.screen5 <- purrr::map(fgsea.clue.screen4 ,~ .x[,3])
...
#save in excel file-type
library(openxlsx)
openxlsx::write.xlsx(x = drug.choose.detail , file = "./result_92742/cp/92742_all_results_with_CM_Drug.xlsx",
                     sheetName = "screenResult", rownames = FALSE)
