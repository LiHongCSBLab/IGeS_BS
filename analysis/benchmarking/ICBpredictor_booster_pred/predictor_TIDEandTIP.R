...
model_icb_tide_TIP <- function(mat,
  ...
  setdiff(PD1_PDL1,rownames(mat))
  if(length(intersect(PD1_PDL1,rownames(mat))) ==1){
    tmp=mat[rownames(mat) %in% intersect(PD1_PDL1, rownames(mat)), ]
    tmp = rbind(tmp, matrix(rep(NA,length(tmp)),1,length(tmp)))
    rownames(tmp) = c(intersect(PD1_PDL1,rownames(mat)), setdiff(PD1_PDL1,rownames(mat)))
  }

  PD1_PDL1sig = data.frame(t(tmp))
  ...
  if(!file.exists(save_path)){dir.create(save_path)}
  TIDE_pre$patient = gsub('\\.', ':', TIDE_pre$patient)
  write.csv(TIDE_pre, paste0(save_path, "TIDE_TIP_result_", cancername,"_",sample_type, ".csv"),
            quote = F) 

  TIDE_pre2=TIDE_pre[,-which(colnames(TIDE_pre) %in% c("class", "TIP_class"))]
  treated_group = TIDE_pre2[TIDE_pre2$patient %in% sample_label[sample_label$treatment == 1, ]$treatment_ID,]
  control_group = TIDE_pre2[TIDE_pre2$patient %in% sample_label[sample_label$treatment == 0, ]$treatment_ID,]
  res = apply(treated_group[,-1], 2, mean) - apply(control_group[,-1], 2, mean)

  write.csv(res, paste0(save_path, "TIDE_TIP_pred.csv"),
            quote = F) 
  # return(TIDE_pre) 
}
