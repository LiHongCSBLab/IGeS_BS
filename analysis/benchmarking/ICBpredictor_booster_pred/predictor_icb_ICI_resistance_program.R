...
#   gene_expression = log2(gene_expression+1)
  ...
  df2=df$res
  rownames(df2) = gsub('\\.', ':',rownames(df2) )

  treated_group = df2[rownames(df2) %in% sample_label[sample_label$treatment == 1, ]$treatment_ID,]
  control_group = df2[rownames(df2) %in% sample_label[sample_label$treatment == 0, ]$treatment_ID,]
  res = apply(treated_group, 2, mean) - apply(control_group, 2, mean)

  write.csv(res, paste0(save_path, "sig_OE_res.csv"), quote = F) 
