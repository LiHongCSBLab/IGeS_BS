...
# try({
    res = lapply(as.list(names(ITS_p_set)), function(x){
        ...
        res = lapply(as.list(names(ITSlist)), function(y){
                    y='oe_nivolumab.resistant.melanoma.up'
                    print(y)
                    glist=ITSlist[[y]]
                    if(length(glist) < 5){
...
names(res) = names(ITS_p_set)
save(res , file=paste0(savepath, "ITS_Reactome_enrichResult_named.Rdata"))
load(file=paste0(savepath, "ITS_Reactome_enrichResult_named.Rdata"))
res_all = do.call(rbind, res)
res_all_sign = res_all[res_all$p.adjust < 0.05, ]

res_summary = lapply(as.list(names(ITS_p_set[[1]])), function(x){
    print(x)
    tmp = as.data.frame(sort(apply(table(res_all_sign[res_all_sign$ITS == x, ][c('cancer', 'Description')]),2,sum)))
    if(nrow(tmp)==0){
        tmp=  data.frame(Number_cancer=NA, reactomePW=NA, IGeS=x)
    }else{
        names(tmp) = "Number_cancer"
        tmp$reactomePW = rownames(tmp)
        tmp$IGeS = x
    }
    return(tmp)
})
names(res_summary) = names(ITS_p_set[[1]])
# openxlsx::write.xlsx(res_summary, paste0(savepath, "IGeS_Func_summary_oe_nivolumab.resistant.melanoma.up.xlsx"))
openxlsx::write.xlsx(res_summary, paste0(savepath, "IGeS_Func_summary.xlsx"))
# # ------------------------------------------------------------------------------
